#!/usr/bin/env python3
"""demo_report.py —— generate a realistic MatchReport, render it to HTML, and save to examples/.

Prefers LocalLLMAnalyzer (vLLM on port 8000) · otherwise falls back to a hardcoded demo Report.

Usage:
    python3 scripts/demo_report.py
    python3 scripts/demo_report.py --fallback        # force-skip vLLM · use the fallback
    python3 scripts/demo_report.py --out my.html     # specify the output path
"""
...
# ensure the src package at the repo root can be imported
...
# ──────────────────────────────────────────────────────────────────────────────
# Synthesize a realistic S17 WorldState sequence for one match
# ──────────────────────────────────────────────────────────────────────────────

def _build_s17_states() -> list[WorldState]:
    """Build a signal-rich WorldState sequence · covering opening -> formed -> closing."""
    ...
    # F01 · 1-2 pve · build up to 2-cost
    states.append(WorldState(
    ...
    # F02 · 2-1 augment · pick an augment
    states.append(WorldState(
    ...
    # F03 · 2-2 pve · picked Mage Power · start saving gold
    states.append(WorldState(
    ...
    # F04 · 2-5 pve · economy peak · win streak saved up to 50g
    states.append(WorldState(
    ...
    # F05 · 3-2 pve · cliff · after leveling to 7, only rolled down to 10g
    states.append(WorldState(
    ...
    # F07 · 4-1 pve · lose streak to save gold · HP critical
    states.append(WorldState(
    ...
    # F08 · 5-1 pvp · stop-loss heavy roll · Kai'Sa hit 3-star
    states.append(WorldState(
...
# ──────────────────────────────────────────────────────────────────────────────
# Hardcoded fallback demo Report
# ──────────────────────────────────────────────────────────────────────────────

def _demo_fallback_report() -> MatchReport:
    """Hardcoded demo MatchReport · used to produce HTML when vLLM is unavailable."""
    return MatchReport(
...
# ──────────────────────────────────────────────────────────────────────────────
# vLLM attempt -> generate MatchReport
# ──────────────────────────────────────────────────────────────────────────────
...
async def _try_vllm_report(
    ...
    """Try calling LocalLLMAnalyzer · returns None on failure."""
    try:
        ...
        # quick check: the summary is not an empty placeholder
        if "LLM 未启动" in report.summary or not report.key_rounds:
            return None
        return report
    except Exception as e:
        log.warning("vLLM call failed · %s", e)
        return None
...
async def _async_main(args: argparse.Namespace) -> None:
    ...
    if not args.fallback:
        log.info("trying vLLM · base_url=%s · model=%s", args.llm_url, args.llm_model)
        report = await _try_vllm_report(args.llm_url, args.llm_model, states)
        if report is not None:
            used_vllm = True
            log.info("vLLM succeeded · match_id=%s · key_rounds=%d", report.match_id, len(report.key_rounds))

    if report is None:
        log.info("using fallback demo report")
    ...
    log.info(
        "HTML written · %s · %.1f KB · vLLM=%s · match_id=%s",
        out_path, size_kb, used_vllm, report.match_id,
    )

    # acceptance output
    print(f"[demo_report] output: {out_path}")
    print(f"[demo_report] size: {size_kb:.1f} KB")
    print(f"[demo_report] source: {'vLLM real inference' if used_vllm else 'hardcoded fallback'}")
...
def main() -> None:
    ap = argparse.ArgumentParser(
        prog="demo_report",
        description="generate a realistic S17 demo replay-analysis HTML",
    )
    ap.add_argument(
        "--out",
    ...
    ap.add_argument(
        "--fallback",
        action="store_true",
        help="skip vLLM and use the fallback demo directly",
    )
    args = ap.parse_args()
